...
def sample_topk(topk_values, topk_indices, temperature=1.0, top_k=None):
    ...
    if top_k is not None:
        top_k = max(1, min(int(top_k), topk_values.shape[-1]))
        topk_values = topk_values[:, :top_k]
...
class GPTSoVITS_ONNX_Inference:
    ...
    def infer(self, ref_wav_path, prompt_text, prompt_lang, text, text_lang,
        ...
        for seg_idx, seg in enumerate(segments):
            ...
            current_samples = sample_topk(topk_values, topk_indices, temperature=temperature, top_k=top_k)

...
def build_default_adapter_definition_store(
    *,
    ...
    enable_qwen3_tts_local: bool | None = None,
    ...
    if _should_enable_qwen3_tts_local(enable_qwen3_tts_local):
        families_by_adapter["qwen3_tts_local"] = [
            FamilyDefinition(
                family_id="qwen3_tts_local_default",
                adapter_id="qwen3_tts_local",
                display_name="Qwen3-TTS Local",
                route_slug="qwen3-tts-local",
                supports_main_models=True,
                supports_submodels=True,
                supports_presets=True,
                auto_singleton_submodel=False,
        ...
        registry.register(
            AdapterDefinition(
                ...
                supported_families=["qwen3_tts_local_default"],
                runtime_kind="local_in_process",
                capabilities=AdapterCapabilities(
                    block_render=True,
                    exact_segment_output=True,
                ...
                asset_topology=AssetTopology(
                    instance_assets=["model_dir"],
                    preset_assets=["reference_audio"],
                ),
                preset_schema={
                    "type": "object",
                    "required": ["preset_id", "display_name"],
                ...
                override_policy=OverridePolicy(
                    ...
                    overridable_fields=["speaker", "language", "instruct", "reference_text", "synthesis.*"],
                ),
                max_concurrent_renders=1,
            )
        )
...
def _should_enable_qwen3_tts_local(explicit: bool | None) -> bool:
    if explicit is not None:
        return explicit
    qwen3_root_env = os.environ.get("NEO_TTS_QWEN3_TTS_ROOT")
    if qwen3_root_env:
        qwen3_root = Path(qwen3_root_env).resolve()
        if qwen3_root.is_dir() and (qwen3_root / "qwen_tts").is_dir():
            return True

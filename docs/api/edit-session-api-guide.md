...
`target_dir` 不是任意文件系统路径。

规则如下：

- 相对路径：解析到受控 export root 下
- 绝对路径：也必须仍然位于该 export root 内
- 任何试图逃逸 export root 的路径都会返回 `400`

因此，前端最好把 `target_dir` 当成“导出命名空间下的相对目录名”，不要传用户机器上的任意绝对路径。
